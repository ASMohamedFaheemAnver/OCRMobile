...
object PostMan {
    ...
    fun getImageResult(uri: Uri, baseContext: Activity, progressDialog: ProgressDialog){
        var textView: TextView = baseContext.findViewById(R.id.show_result)
        var client = SyncHttpClient()
        var params = RequestParams()

        path = uri?.let { PathGenerator.getPath(baseContext, it) }!!
    ...
    fun postData(progressDialog: ProgressDialog){
        var client = SyncHttpClient()
        var params = RequestParams()
